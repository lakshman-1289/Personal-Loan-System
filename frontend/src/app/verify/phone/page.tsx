...
export default function VerifyPhone() {
  ...
  const sendOtp = async () => {
    ...
    setSending(true);
    try {
      const response = await axios.post('/api/v1/verification/phone/send');
      setInfo(response.data.message || 'Phone verification OTP sent successfully!');
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to dispatch phone verification OTP.');
    } finally {
      setSending(false);
    }
  };
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('/api/v1/verification/phone/verify', { token });
      setInfo(response.data.message || 'Phone verified successfully!');
  ...
  return (
    <div className="max-w-md mx-auto my-12 bg-white border border-[#E4EAF0] p-8 rounded-2xl shadow-lg relative overflow-hidden text-neutral-text">
      ...
      <div className="mb-8 text-center space-y-3">
        ...
        <h2 className="text-3xl font-extrabold text-neutral-text tracking-tight">Verify Your Mobile</h2>
        <p className="text-neutral-secondary text-xs leading-relaxed">
          Please confirm your mobile number. We need to verify this communication channel before proceeding to the KYC steps.
        </p>
      </div>
