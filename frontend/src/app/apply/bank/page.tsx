  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-8 py-6 text-neutral-text">
      ...
      <div className="flex items-center justify-between text-xs text-neutral-secondary bg-brand-light-blue border border-[#E4EAF0] rounded-full px-6 py-3">
        ...
        <ChevronRightIcon className="h-3 w-3" />
        <span className="text-brand-blue font-bold">4. Bank details</span>
        <ChevronRightIcon className="h-3 w-3" />
        <span>5. Sign Declarations</span>
      </div>

      <div className="bg-white border border-[#E4EAF0] p-8 rounded-2xl shadow-lg">
        <h2 className="text-2xl font-bold text-neutral-text tracking-tight mb-2">Disbursement Bank Account</h2>
        <p className="text-neutral-secondary text-xs mb-8">Enter your banking coordinates where approved loan funds will be transferred.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="block text-xs font-semibold uppercase tracking-wider text-neutral-secondary">Bank Account Number</label>
              <input
                type="text"
                required
                value={accountNumber}
            ...
            <div className="space-y-2">
              <label className="block text-xs font-semibold uppercase tracking-wider text-neutral-secondary">IFSC Code</label>
              <input
                type="text"
                required
                value={ifscCode}
                onChange={(e) => setIfscCode(e.target.value)}
                className="w-full bg-white border border-[#E4EAF0] text-neutral-text rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-brand-green focus:ring-1 focus:ring-brand-green transition duration-200 font-mono uppercase"
                placeholder="e.g. HDFC0001234"
              />
            </div>

            <div className="space-y-2">
              <label className="block text-xs font-semibold uppercase tracking-wider text-neutral-secondary">Account Type</label>
              <select
                value={accountType}
            ...
            <div className="space-y-2">
              <label className="block text-xs font-semibold uppercase tracking-wider text-neutral-secondary">Branch Name</label>
              <input
                type="text"
                required
                value={branchName}
